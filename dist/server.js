...
import { createSearchDocumentsTool, createGetDocumentByIdTool, createGetDocumentsByCategory } from "./tools/document-search.tools.js";
// Project ID 파싱
...
async function main() {
    ...
    // Register tools
    server.tool("search-documents", "AIApp BaaS 인증 시스템 문서를 검색합니다. API 규격, 구현 예제, 보안 가이드, 프레임워크별 템플릿을 키워드로 검색할 수 있습니다.", searchDocumentsTool.inputSchema, searchDocumentsTool.handler);
    server.tool("get-document-by-id", "문서 ID로 특정 BaaS 인증 시스템 문서의 전체 내용을 조회합니다.", getDocumentByIdTool.inputSchema, getDocumentByIdTool.handler);
    server.tool("get-documents-by-category", "카테고리별로 BaaS 인증 시스템 문서를 조회합니다. API, 템플릿, 보안, 예제, 개발 가이드 등의 카테고리로 필터링할 수 있습니다.", getDocumentsByCategoryTool.inputSchema, getDocumentsByCategoryTool.handler);
    // Project configuration tool
    server.tool("get-project-config", "현재 MCP 서버에 설정된 프로젝트 ID를 확인합니다.", {
        type: "object",
        properties: {}
    }, async () => ({
        content: [{
